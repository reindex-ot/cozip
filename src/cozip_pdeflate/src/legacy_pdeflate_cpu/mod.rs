...
pub(crate) fn pdeflate_compress_chunk_payload_cpu(
    chunk: &[u8],
    options: &PDeflateOptions,
) -> Result<PDeflateChunkPayload, PDeflateError> {
    validate_options(options)?;
    let mut cpu_opts = options.clone();
    cpu_opts.gpu_compress_enabled = false;
    let compressed = compress_chunk(chunk, &cpu_opts)?;
...
pub(crate) fn pdeflate_compress_chunks_payload_gpu_batch(
    chunks: &[&[u8]],
    options: &PDeflateOptions,
...
pub(crate) fn pdeflate_compress_chunks_payload_gpu_batch_with_breakdown(
    ...
    let same_len = chunks
        .first()
        .map(|first| chunks.iter().all(|chunk| chunk.len() == first.len()))
        .unwrap_or(false);

    if same_len {
        let chunk_size = chunks[0].len().max(1);
        let mut input = Vec::with_capacity(chunk_size.saturating_mul(chunks.len()));
        for &chunk in chunks {
            input.extend_from_slice(chunk);
        }
        let indices: Vec<usize> = (0..chunks.len()).collect();
        ...
        let mut finalized = Vec::with_capacity(chunks.len());
...
pub(crate) fn pdeflate_pack_stream_from_chunk_payloads(
    ...
    let mut out = Vec::with_capacity(
        24usize
            .saturating_add(payloads.len().saturating_mul(4))
            .saturating_add(payloads.iter().map(|p| p.len()).sum::<usize>()),
    );
    ...
    write_u64_le(
        &mut out,
        u64::try_from(input_len).map_err(|_| PDeflateError::NumericOverflow)?,
    );
    write_u32_le(
        &mut out,
        u32::try_from(payloads.len()).map_err(|_| PDeflateError::NumericOverflow)?,
    );
    for payload in payloads {
        write_u32_le(
            &mut out,
            u32::try_from(payload.len()).map_err(|_| PDeflateError::NumericOverflow)?,
        );
